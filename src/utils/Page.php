...
class Page
{
    public const URL_NORMAL = 1;   //常规url访问，用 ? 传参

    public const URL_STATIC = 2;   //伪静态路径访问，用 - 传参
    ...
    public function showPageHandle($style = DEFAULT_PAGE_STYLE)
    {
        ...
        $html = '';
        if ($style & PAGE_TOTAL_NUM) {
            $html .= '<li class="disabled"><a href="#">总记录：'.$this->totalRows.'</a></li>';
        }
        if ($style & PAGE_PREV) {
            $html .= $this->prevPage();
        }
        if ($style & PAGE_LIST) {
            $html .= $this->printPageList($style);
        }
        if ($style & PAGE_NEXT) {
            $html .= $this->nextPage();
        }
        if ($style & PAGE_INPUT) {
            $html .= $this->printInput();
        }
    ...
    {
        if ($this->pageNum <= 1 && $showFirstPage == false) {
            return [];
        }

        $pages = ['url' => $this->buildUrl($this->url.'{page}')];
        if ($style & PAGE_TOTAL_NUM) {
            $pages['total'] = $this->totalRows;
        }
    ...
    private function buildUrl($url)
    {
        if ($this->urlPathType == self::URL_NORMAL) {
            return $url;
        } elseif ($this->urlPathType == self::URL_STATIC) {
            return url($url);
        }
    }
}
