...
class Redis
{
    ...
    private function __construct(array $config = [])
    {
        if (!extension_loaded('redis')) {
            throw new HeroException('please install redis extension!');
        }
        $redisConfig = array_merge(Config::get('redis', null, []), $config);
        if (empty($redisConfig)) {
            throw new HeroException('redis config empty!');
        }
        $this->_redis = new \Redis();
        $result = $this->_redis->connect($redisConfig['host'], $redisConfig['port'], $redisConfig['timeout']);
        if ($result === false) {
            throw new HeroException('redis connect fail');
        }
        if ($redisConfig['auth']) {
            $this->_redis->auth($redisConfig['auth']);
        }
        $this->_redis->select($redisConfig['database']);
